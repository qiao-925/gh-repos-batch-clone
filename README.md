...
# 检查失败列表中的仓库
repos check -f failed-repos.txt

# 检查所有仓库（使用 10 个并行任务，加快检查速度）
repos check -t 10
```
...
2. **单独检查**（方案3）：使用 `repos check` 命令，只检查已存在的仓库
   - 不执行克隆，只检查
   - 可以随时验证已存在的仓库
   - 适合定期检查或修复后验证
...
- **单独检查模式**：使用 `repos check` 命令，只检查已存在的仓库，不执行克隆

> 💡 **提示**：所有检查命令示例已集中在[第二章：常用命令速查](#二常用命令速查)，可直接查看使用。
...
开始 (main.py)
  ...
  │
  ├─→ [4] 执行模式判断
  │     ├─ 如果命令是 check：跳转到 [5] 只检查模式
  │     └─ 否则（命令是 clone）：继续执行 [4] 克隆模式
  │
  ├─→ [4] 并行执行克隆（正常模式）
